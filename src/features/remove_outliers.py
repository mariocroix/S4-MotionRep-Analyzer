...
# adds a new col tot he original one with true/false outliers
col = "acc_x"
...
# the rest's data wasn't normally distributed,
# that's why it was showing so many outliers

# --------------------------------------------------------------
# Chauvenets criteron (distribution based)
# --------------------------------------------------------------

# Check for normal distribution since chauv. crit. requires it
# histograms should be bell curved
df[outlier_columns[:3] + ["label"]].plot.hist(by="label", figsize=(20,20), layout=(3,3))
df[outlier_columns[3:] + ["label"]].plot.hist(by="label", figsize=(20,20), layout=(3,3))
plt.show()

# Insert Chauvenet's function
# returns the same as the iqr method
# the only difference is the way of calcs
...
# Loop over all columns
# chauv's method shows less outliers which is better
...
# here you can see the difference between distribution and distance based methods
# --------------------------------------------------------------
...
# we won't use this iqr method, since it is too strict and we'll throw away a lot of data
for col in outlier_columns:
    dataset = mark_outliers_chauvenet(df[df["label"] == label], col)
    plot_binary_outliers(dataset, col, col + "_outlier", reset_index=True)
# chav methos treated data very nicely
dataset, outliers, X_scores = mark_outliers_lof(df[df["label"] == label], outlier_columns)
for col in outlier_columns:
    plot_binary_outliers(dataset=dataset, col=col, outlier_col="outlier_lof", reset_index=True)
# with lof methos, the red points are marked with the blue dots, not necessarily below or above them
...
for col in outlier_columns:
    for label in df["label"].unique():
        ...
        # how many outliers are there for that specific exercice
        n_outlines = len(dataset) - len(dataset[col].dropna())
        print(f"Removed {n_outlines} from {col} for {label}")
